...
```bash
pytest pytest tests/test_chat.py
```
## Тестирование корректной работы парсинга Google таблицы
```bash
pytest pytest tests/test_new_structure.py
```
Для подробного вывода по всем кейсам :
```bash
pytest pytest tests/test_chat.py -v -s
```
```bash
pytest pytest tests/test_new_structure.py -v -s
```
