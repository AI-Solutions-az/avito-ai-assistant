...
async def process_and_send_response(message: WebhookRequest):
    ...
    if message.payload.value.author_id == message.payload.value.user_id:
        if (re.search('оператор', message_text, re.IGNORECASE) or
            ...
            await send_alert(f"‼️Требуется внимание менеджера:\n"
                             f"Объявление: {ad_url}\n"
    ...
    user_name, user_url = await get_user_info(message.payload.value.user_id, message.payload.value.chat_id)
    ad_url = await get_ad(message.payload.value.user_id, message.payload.value.item_id)
    response = await process_message(message.payload.value.author_id, message.payload.value.chat_id,
    ...
    if response:
        ...
        await send_alert(f"💁‍♂️ {user_name}: {message.payload.value.content.text}\n"
                         f"🤖 Бот: {response}\n"
                         f"_____\n\n"
                         f"💬 Диалог: https://www.avito.ru/profile/messenger/channel/{message.payload.value.chat_id}", thread_id=0)
        ...
        if (re.search('оператор', message.payload.value.content.text, re.IGNORECASE) or
            ...
            await add_chat(message.payload.value.chat_id)
            return None
    else:
        return None
